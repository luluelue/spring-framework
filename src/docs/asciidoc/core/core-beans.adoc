...
The preceding `WebConfig` class is equivalent to the following Spring `<beans/>` XML:
...
assuming that `WebConfig` is declared within the `com.acme` package (or any package
...
container. In this series examples, we create a `@Configuration` class named `WebConfig` and
...
`@Configuration` annotation and processes the `@Bean` methods declared in `WebConfig`
properly.
...
NOTE: In `system-test-config.xml` file, the `WebConfig` `<bean/>` does not declare an `id`
